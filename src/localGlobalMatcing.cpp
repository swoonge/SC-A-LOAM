...
// Publisher 선언
ros::Publisher pubLCdetectResult, pubKeyPointResult, pubKeyPointDisplay, pubKeyPointSurround, pubKeyregionDisplay, pubGlobalGraph, pubLocalGraph, pubGlobalGraphDisplay, pubLocalGraphDisplay;
...
//process methods  
void KeypointMatchingProcess( void ) {
    float frequency = 0.5; // can change 
    ros::Rate rate(frequency);
    while (ros::ok()){
        if (KPwithInfo.size() < 20) {
            rate.sleep();
            continue;
        }

        size_t currentIdx = 0;
        for (size_t i = ProcessedDescriptorCount; i > ProcessedDescriptorCount - 20; i--) {
            if (currentIdx < get<2>(KPwithInfo[i])) {
                currentIdx = get<2>(KPwithInfo[i]);
            }
        }
        
        size_t boundaryIdx = 0;
        geometry_msgs::Pose currentPose = get<2>(keyPoseIdxPointCloudGFVector[currentIdx]);
        for (int i = currentIdx - 1; i >= 1; i--) {
            ...
            if (distance > LocalMapBoundary * localGraphMu) {
                boundaryIdx = i;
                break;
            }
        }
        ...
        aloam_velodyne::MapAndDescriptors globalmsg;
        sensor_msgs::PointCloud2 globalKPmsg;
        aloam_velodyne::Float64MultiArrayArray globalDmsg;
        aloam_velodyne::MapAndDescriptors localmsg;
        sensor_msgs::PointCloud2 localKPmsg;
        aloam_velodyne::Float64MultiArrayArray localDmsg;       
        /////durl
        pcl::PointCloud<pcl::PointXYZI> localKPPC;
        pcl::PointCloud<pcl::PointXYZI> globallKPPC;
        globalmsg.size = 0;
        localmsg.size = 0;

        
        for (size_t i = 0; i < KPwithInfo.size(); i++) {
            if (get<2>(KPwithInfo[i]) < boundaryIdx) { 
                globallKPPC.points.push_back(get<0>(KPwithInfo[i])->points[0]);
                std_msgs::Float64MultiArray msgsCache;
                for (size_t j = 0; j < 135; j++) {
                    msgsCache.data.push_back(get<3>(KPwithInfo[i]).histogram[j]);
                }
                globalDmsg.descriptor.push_back(msgsCache);
                globalmsg.size++;
            }
            else {
                localKPPC.points.push_back(get<0>(KPwithInfo[i])->points[0]);
                std_msgs::Float64MultiArray msgsCache;
                for (size_t j = 0; j < 135; j++) {
                    msgsCache.data.push_back(get<3>(KPwithInfo[i]).histogram[j]);
                }
                localDmsg.descriptor.push_back(msgsCache);
                localmsg.size++;
            }
        }

        if (localKPPC.points.size() < 1) {
            rate.sleep();
            continue;
        }

        pcl::toROSMsg(globallKPPC, globalKPmsg);
        pcl::toROSMsg(localKPPC, localKPmsg);
        globalKPmsg.header.frame_id = "/camera_init";
        localKPmsg.header.frame_id = "/camera_init";
        pubGlobalGraphDisplay.publish(globalKPmsg);
        pubLocalGraphDisplay.publish(localKPmsg);

        globalmsg.descriptors = globalDmsg;
        globalmsg.keypoints = globalKPmsg;
        
        localmsg.descriptors = localDmsg;
        localmsg.keypoints = localKPmsg;

        pubLocalGraph.publish(localmsg);
        pubGlobalGraph.publish(globalmsg);
        // 점들을 현재 pose기준 일정거리 이하의 keypose의 점들을 로컬로, 나머지를 글로벌로 구성하여 point와 discriptor를 보냄.
        rate.sleep();
    }
}
    ...
    while (ros::ok()){
        ...
        if (keyPoseIdxPointCloudGFVectorSize > keyPointExtractionCount) {
            ...
            for (size_t i = 0; i < currkeypoints->size(); i++) {
                print(currkeypoints->points[i].intensity);
            }
...
int main(int argc, char **argv)
{
    ...
    nh.param<float>("localGraphMu", localGraphMu, 1.5);
    ...
    pubGlobalGraph = nh.advertise<aloam_velodyne::MapAndDescriptors>("/globalGraph", 100);
    pubLocalGraph = nh.advertise<aloam_velodyne::MapAndDescriptors>("/localGraph", 100);
    pubGlobalGraphDisplay = nh.advertise<sensor_msgs::PointCloud2>("/globalGraphDisplay", 100);
    pubLocalGraphDisplay = nh.advertise<sensor_msgs::PointCloud2>("/localGraphDisplay", 100);
    ...
    std::thread threadKPMatching(KeypointMatchingProcess);
